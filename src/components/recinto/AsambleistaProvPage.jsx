...
const RecintoSeleccionado = () => {
    ...
    const handleVotoSubmit = () => {
        if (votoInput.trim() === "") {
            alert("Por favor, ingrese una cantidad de votos.");
            return;
        }
        alert(`Se han registrado ${votoInput} votos`);
        setVotoInput("");
    ...
    return (
        <div className="w-full h-full p-5 bg-white rounded-[15px] relative">
            ...
            <div className="w-[20%] max-lg:flex-col items-center mt-8">
                {/* Primer filtro */}
                <div className="w-full items-center border border-black rounded-md overflow-hidden">
                    <div className="p-2 bg-[#4880FF] border-r border-black">
                        <input
                            type="text"
                            placeholder="Buscar partido..."
                            className="text-left bg-[#4880FF]"
                            style={{ appearance: 'none', color: 'white', width: '100%' }}
                            value={inputValue}
